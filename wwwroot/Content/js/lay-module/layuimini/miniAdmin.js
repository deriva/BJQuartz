...
layui.define(["jquery", "miniMenu", "element", "miniTab", "miniTheme"], function (exports) {
    ...
    var miniAdmin = {
        ...
        render: function (options) {
            ...
            options.localmenus = options.localmenus || 1;

            if (options.localmenus == 1) {
                $.getJSON(options.iniUrl, function (r) {
                   
                    miniAdmin.renderMenus(r, options);
                ...
                var url2 = "SystemModuleV2Service/GetMenus";
                options.iniUrl = url2; 
                http.post2(url2, { roles: "", appcode:"lwdb"}).then(r => {
                    var data = r.attr; 
                    if (!r.attr) {
                        data = r;
                    }
                    miniAdmin.renderMenus(data, options); 
                }, err => {
                    miniAdmin.error('菜单接口有误');
                    //	window.location.href = "/account/vn/login.html?appcode=bjcenter";
                });
            }

        },
        renderMenus: (data, options) => {
        ...
        renderLogo: function (data) {
            var html = '<a href="' + data.href + '"><img src="' + data.image + '" alt="logo"><h1>' +
                data.title + '</h1></a>';
        ...
        renderHome: function (data) {
            ...
            $('#layuiminiHomeTabId').html(
                '<span class="layuimini-tab-active"></span><span class="disable-close">' + data
                    .title + '</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>');
        ...
        listen: function () {
            ...
            $('body').on('click', '[data-refresh]', function () {
                $(".layui-tab-item.layui-show").find("iframe")[0].contentWindow.location
                    .reload();
