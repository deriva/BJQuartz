...
class Ban {
    ...
    _pushWatcher(watcher) {
        ...
        var id = watcher.id;
        var index = this._binding[watcher.key].findIndex(x => x.id == id && x.key == watcher.key && x.attr == watcher.attr);
        if (index == -1)
    ...
    _getNode(keyname) {
        if (this._binding[keyname]) {
            var item = this._binding[keyname].find(x => x.key == keyname);
     ...
     * @param {any} key 属性名称
     * @param {any} val 属性值
     * @param {any} isremove 是否删除属性
     */
    _setAttr(keyname, key, val, isremove) {
        var item = this._getNode(keyname);
        if (item) {
    ...
    /**
     * @param {any} obj 对象
     * @param {any} tmpid 模板id
     * @param {any} htmlid 渲染后的id
     */
    _tplParse(obj, tmpid, htmlid) {
   ...
   */
    ...
    * @param {any} key  是否显示
    */
    _SetName(obj, lst, key) {
        var val = obj[key];
        if (val == undefined) return;
        var indx = lst.findIndex(y => "-" + y.key.toString() == "-" + val);
        if (indx > -1) {
            var desc = lst[indx].val;
            if (desc.indexOf("请选择") > -1) desc = "-";
            obj[key + "Name"] = desc;
            obj[key + "Index"] = indx;
        }
    }
...
var bfUnilt = {
    ...
    IDGenerate: () => {//id生成器
        idStartNo++;
        return "ban" + idStartNo;
    },
    NodeSetID(node) {
        var id = node.getAttribute("id");
        if (id == undefined || id == null || id == "") {
            node.setAttribute("id", bfUnilt.IDGenerate());
        }
    }
}
...
function _compile(el, self, elid) {
    ...
    [...child].forEach((node) => {
        ...
        if (node.nodeType === 3) {
            ...
            bfUnilt.NodeSetID(node);//每个节点设置id
...
function _compile_bfmodel(node, self, attr) {
    ...
    var val = bfUnilt._getVmVal(self.$data, keyName);
    // if (attr.hasOwnProperty('bf-compiled')) return;//如果有编译标记的直接返回(防止重复注册事件)
    ...
    if (node.tagName == "INPUT" || node.tagName == "TEXTAREA") {
        ...
        let handle = (e) => {
            ...
            if (node.type == "checkbox" || node.type == "radio") {
                ...
                if (valArr[0] == "") valArr = [];
                var dataval = node.getAttribute("data-val").toString();
                var chked = $(node).prop("checked");
                var bantext = node.getAttribute("ban-text");
           
                var chkCount = 3;
                let deleteIndex = valArr.findIndex(item => item === val);
                if (bantext && bantext.length > 0) {//复选框 单选
                    var bantextArr = bantext.split('|');
                    chkCount = bantextArr.length;
                    if (chkCount <= 2) {
                        if (chked) {
                            valArr = [val];
                        }
                        else {
                            var r = bantextArr.find(x => x != val);
                            valArr = [r];
                        }
                    }
                }
                if (chkCount > 2) {//复选框 多选
                    if (chked) {
                        if (deleteIndex == -1) valArr.push(val);
                    }
                    else {
        ...
        if (node.getAttribute("click")) { }
            ...
            $(node).on('change', handle);//只监听变更事件

            //node.addEventListener('input', handle, false);
            // node.addEventListener('change', handle, false);
        }
    }

    else if (node.tagName == "SELECT") {

        var keyName2 = node.getAttribute("bf-datasource");
        if (keyName2) {
            var arr = bfUnilt._getVmVal(self.$data, keyName2);
            if (arr.length > 0) {
                arr.forEach((x, i) => {
                    node.options[i] = new Option(x.val, x.key);
                });
            }
        }
        node.value = val;
    ...
    } else if (node.tagName == "IMG" || node.tagName.toLowerCase() == "iframe") {
        ...
        if (this.attr == "html")
           ...
           // this.node.value = vv;
            ...
            if (deleteIndex > -1) {
                ...
                $(this.node).prop("checked", false); this.node.removeAttribute("checked");
            }
        }
